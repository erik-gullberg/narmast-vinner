...
export default function Home() {
  ...
  // press "Börja gissa". This collapses mode/length/timer/name into the fixed
  // SOLO_GAME preset; the one screen that is left is the rules, shown from a
  // single-player perspective instead of the "share this code" multiplayer copy.
        ...
        {showSoloRules ? (
          <>
            ...
            <div className="bg-gray-50 rounded-lg p-6 mb-6">
              <h2 className="text-gray-700 text-lg font-bold mb-4 text-center">Regler</h2>
              <ul className="text-center space-y-2">
                <li className="text-gray-600">• Du får se en bild av en händelse eller plats</li>
                <li className="text-gray-600">
                  • Du har {SOLO_GAME.guessTimeSeconds} sekunder på dig att placera ut händelsen på en världskarta
                </li>
                <li className="text-gray-600">• 1000 poäng för fullträff — ju närmare du gissar, desto mer poäng</li>
                  <li className="text-gray-600">• Försök få så mycket poäng som möjligt på  {SOLO_GAME.maxRounds} rundor!
                  </li>
              </ul>
            </div>
            ...
            <button
              ...
              className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-5 px-6 rounded-lg transition-colors text-xl touch-manipulation disabled:opacity-60 flex items-center justify-center gap-2 mb-3"
            >
              {startingSolo ? (
                <>
            ...
            <button
              onClick={() => setShowSoloRules(false)}
              disabled={startingSolo}
              className="w-full text-gray-600 hover:text-gray-800 font-medium py-2 disabled:opacity-60 touch-manipulation"
            >
              Tillbaka
            </button>
          </>
          ...
          <>
            ...
            <div className="space-y-4">
              <button
                onClick={() => setShowSoloRules(true)}
                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-4 px-6 rounded-lg transition-colors text-lg touch-manipulation flex items-center justify-center gap-2"
              >
                Spela själv
              </button>
